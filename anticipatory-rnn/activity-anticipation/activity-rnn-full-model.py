...
def jointModel(num_sub_activities, num_affordances, inputJointFeatures, inputHumanFeatures,inputObjectFeatures):
...
def jointModelVectors(num_sub_activities, num_affordances, inputJointFeatures, inputHumanFeatures,inputObjectFeatures):
    shared_input_layer = TemporalInputFeatures(inputJointFeatures)
    ...
    if not use_pretrained:
        ...
        if architectures[train_for] == 'detection':
            ...
            rnn.fitModel(X_tr_human_shared, X_tr_human_disjoint, Y_tr_human_anticipation,
                         ...
                         '{1}/{0}/'.format(index, path_to_checkpoints), epochs, batch_size, learning_rate_decay,
                         decay_after)
            ...
            rnn = jointModelOutput(num_sub_activities, num_affordances, num_sub_activities_anticipation,
                                   num_affordances_anticipation, inputJointFeatures, inputHumanFeatures,
                                   inputObjectFeatures)
        ...
        if train_more:
            rnn.fitModel(X_tr, Y_tr, 1, '{1}/{0}/'.format(index, path_to_checkpoints), epochs, batch_size,

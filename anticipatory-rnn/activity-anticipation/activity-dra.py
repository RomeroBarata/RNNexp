...
def DRAmodel(nodeList, edgeList, edgeListComplete, edgeFeatures, nodeFeatures, nodeToEdgeConnections,
             clipnorm=25.0, train_for='joint'):
    edgeRNNs = {}
    edgeTypes = edgeList
    ...
    for et in edgeTypes:
        ...
        edgeRNNs[et] = [TemporalInputFeatures(inputJointFeatures),
                        LSTM('tanh', 'sigmoid', lstm_init, truncate_gradient=4, size=args.edgeRNN_size, rng=rng)]  # 128
    ...
    dra = DRAanticipation(edgeRNNs, nodeRNNs, outputLayer, nodeToEdgeConnections, edgeListComplete, softmax_loss,
                          nodeLabels, learning_rate, clipnorm, train_for=train_for)
    return dra


def DRAmodelnoedge(nodeList, edgeList, edgeListComplete, edgeFeatures, nodeFeatures, nodeToEdgeConnections,
                   clipnorm=25.0, train_for='joint'):
    edgeRNNs = {}
    edgeTypes = edgeList
    lstm_init = 'orthogonal'
...
if __name__ == '__main__':
    ...
    dra = []
    if args.noedgeRNN:
        dra = DRAmodelnoedge(nodeList, edgeList, edgeCompleteList, edgeFeatures, nodeFeatures,
                             nodeToEdgeConnections, train_for=train_for)
    else:
        dra = DRAmodel(nodeList, edgeList, edgeCompleteList, edgeFeatures, nodeFeatures,
                       nodeToEdgeConnections, train_for=train_for)
    ...
    dra.fitModel(trX, trY, 10, checkpoint_dir, epochs=10, std=1e-5, trX_validation=trX_validation,
                 trY_validation=trY_validation, predictfn=OutputMaxProb, maxiter=1000,
                 train_for=train_for, learning_rate=np.float32(0.001))
